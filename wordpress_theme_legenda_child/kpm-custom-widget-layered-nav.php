 ...
 * @extends  WC_Widget
 */
class KPM_Custom_Widget_Layered_Nav extends WC_Widget {
	...
	public function init_settings() {
        ...
        function get_product_count($term_id, $taxonomy){
            $args = array(
                'post_type'             => 'product',
                'post_status'           => 'publish',
			...
			if ( 'dropdown' == $display_type ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					foreach ( $terms as $term ) {
                                                $public_product_list = $this->get_product_count($term->term_id, $taxonomy);
						...
						if ( 'and' == $query_type ) {
                                                        ...
                                                        $filtered = array_intersect( $_products_in_term, WC()->query->filtered_product_ids );
                                                        $filtered_more = array_intersect( $public_product_list, $filtered );
                                                        $count = count( $filtered_more );
				...
				foreach ( $terms as $term ) {
					...
					// If this is an OR query, show all options so search can be expanded
                                                ...
                                                $filtered = array_intersect( $_products_in_term, WC()->query->unfiltered_product_ids );
                                                $filtered_more = array_intersect( $public_product_list, $filtered );
						$count = count( $filtered_more );
